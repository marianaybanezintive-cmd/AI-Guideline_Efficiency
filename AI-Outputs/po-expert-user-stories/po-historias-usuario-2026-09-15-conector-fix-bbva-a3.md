...
| **§9 Spikes** | 2026-09-16 | 4 resueltos (`S-01` Drop Copy, `S-02` credenciales fuera de banda, `S-12` IBM WebSphere Liberty, `S-15` homologación parcial aceptada por A3) · 11 mantenidos como pendientes a confirmar con el equipo técnico o el negocio, registrados como notas en las historias afectadas · 1 nuevo emergente (`S-16`) |

Las resoluciones de la segunda pausa modificaron el contenido: se agregaron las reglas **RN-17** (oyente pasivo en Drop Copy) y **RN-18** (*hard stop* de credenciales), los códigos **MSG-23** y **MSG-24**, y criterios de aceptación nuevos en `FIX-2.01` y `FIX-2.04`. El detalle está en §9.1.
...
| **SUP-14** | **Los números de secuencia se diseñan de forma configurable** entre reinicio diario y continuidad entre jornadas, porque `ResetSeqNumFlag` (141) no está documentado en el ROE de A3 y la respuesta de A3 aún no está disponible | ✅ Confirmado por el PO, sin cambios (2026-09-15) |
...
| **RN-06** | **Las credenciales de A3 nunca se registran** en logs, trazas, mensajes de error ni en el repositorio. El conector tiene acceso de sólo lectura al secret administrado por seguridad de BBVA | Propuesta técnica, «Manejo de contraseñas» |
...
| **MSG-19** | Error | Credenciales no disponibles | "No se pudo leer el secret de credenciales de A3. La sesión no se inicia." |
...
| **Depende de** | `T-01` (ambiente intive), `T-05` (credenciales y coordenadas A3), `T-06` (secret de credenciales) |
...
4. **[Feliz]** Las credenciales se obtienen del secret administrado por seguridad de BBVA en el momento de la conexión, y no quedan registradas en ningún log ni traza, conforme a RN-06.
...
8. **[Error]** Si no se puede leer el secret de credenciales, el conector no intenta la conexión y publica MSG-19: "No se pudo leer el secret de credenciales de A3. La sesión no se inicia."
...
Característica: Establecimiento de la sesión FIX con A3
  ...
  Escenario: Reanudación tras actualizar la credencial
    ...
    Y que un administrador actualizó la contraseña en el secret de BBVA
  ...
  Escenario: Secret de credenciales no disponible
    Dado que el proveedor de secretos de BBVA no responde
    Cuando el conector intenta iniciar la sesión
    Entonces no se establece la conexión TCP
    Y se publica el error MSG-19: "No se pudo leer el secret de credenciales de A3. La sesión no se inicia."
...
- **Pendiente de confirmar con el equipo técnico (`S-03`):** el comportamiento de los números de secuencia al abrir la sesión diaria. El diseño los deja configurables conforme a SUP-14.
- **Resuelto el 2026-09-16 (`S-02`):** la actualización de la credencial tras un *hard stop* implica que un administrador la modifique manualmente —en reMarkets a través del soporte de Primary, en producción desde las consolas administrativas de Primary— y luego la cargue en el secret o bóveda de BBVA. El procedimiento operativo se documenta en `T-06`.
...
- **Pendiente de confirmar con el negocio (`S-06`):** la periodicidad y los criterios de las consultas. ¿Por cuenta al cierre de jornada? ¿Por símbolo bajo demanda? ¿Ambas? La propuesta base es consulta por cuenta al cierre de jornada más consulta bajo demanda por símbolo.
- **Pendiente de confirmar con A3 (`S-05`):** si se puede suscribir a Trade Capture Reports en lugar de consultarlos. El ROE documenta `SubscriptionRequestType` sólo para *allocations y giveups*, que está fuera de alcance. El diseño base asume *pull* periódico.
...
- **Pendiente de confirmar con el equipo técnico (`S-07`):** quién dispara la consulta en la Fase 1. Con el MVP puramente saliente (SUP-04), la propuesta es que la dispare el propio conector según su planificación configurable, no BBVA.
...
> Observación literal del Excel (`Obs`, US 4): `Validar con BBVA donde se persiste (storage?)` — resuelta como SUP-05 (almacenamiento en archivos) y mantenida como `S-10` para confirmación formal.
...
4. **[Feliz]** La persistencia se resuelve mediante almacenamiento en archivos sobre volumen persistente, conforme a SUP-05.
5. **[Alternativo]** El comportamiento al abrir la sesión diaria —reiniciar la numeración o continuarla— es **configurable**, conforme a SUP-14, porque `ResetSeqNumFlag` (141) no está documentado en el ROE de A3.
...
6. **[Validación]** La ventana de deduplicación se acuerda con BBVA; la propuesta del PO es la jornada de negociación completa.
...
| **T-06** | — | Esquema de gestión del secret y procedimiento de credenciales | Definir con seguridad de BBVA la estructura del secret y **documentar el procedimiento administrativo de actualización de contraseña**, que es fuera de banda conforme a `S-02` | Secret de un único campo con la contraseña vigente, accesible en sólo lectura por el conector. Procedimiento documentado con los dos caminos: en reMarkets, solicitud al soporte de Primary; en producción, consola administrativa de Primary gestionada por la ALyC. Incluye el **runbook de salida del *hard stop*** de RN-18: actualizar credencial → cargar en el secret → acción manual de reanudación |
...
| **S-03** | ROE A3 | **¿Cómo se manejan los números de secuencia entre jornadas?** ¿Se reinician a 1 al abrir la sesión diaria o continúan? `ResetSeqNumFlag` (141) no está documentado en el ROE | Afecta el diseño de la persistencia y de la detección de gaps. **Impacta `FIX-2.09` y `FIX-2.10`** | Consultar a A3 junto con S-02. Mientras tanto, diseñar la persistencia con ambos comportamientos configurables, conforme a SUP-14, para no quedar bloqueados por la respuesta | ⏳ **Pendiente (2026-09-16).** El PO mantiene la propuesta y la deja a confirmar con el equipo técnico o con el negocio durante la Épica 1. Registrado como nota en las historias afectadas |
| **S-04** | Análisis del PO | **¿La normalización de ER y TCR requiere enriquecer con datos del instrumento?** Ambos mensajes traen `Symbol` como string | Si la respuesta es sí, hay que **adelantar `FIX-5.05` (SecurityList) al MVP**. Es sólo lectura, así que no rompe el corte, pero agrega alcance | Definirlo al cerrar los contratos MQ en `T-03`. Recomendación: si los consumidores de BBVA necesitan más que el símbolo (moneda, tipo, vencimiento, segmento), adelantar `SecurityList` con carga diaria en memoria | ⏳ **Pendiente (2026-09-16).** El PO mantiene la propuesta y la deja a confirmar con el equipo técnico o con el negocio durante la Épica 1. Registrado como nota en las historias afectadas |
| **S-05** | ROE A3 | **¿Se puede suscribir a Trade Capture Reports en lugar de consultarlos?** El ROE documenta `SubscriptionRequestType` (263) sólo para la variante de *allocations y giveups*, que está fuera de alcance | Define si el flujo de TCR es *pull* periódico o *push*. Cambia el diseño de `FIX-2.05` | Consultar a A3. Asumir *pull* periódico como diseño base, que es lo que el ROE documenta para operaciones regulares | ⏳ **Pendiente (2026-09-16).** El PO mantiene la propuesta y la deja a confirmar con el equipo técnico o con el negocio durante la Épica 1. Registrado como nota en las historias afectadas |
| **S-06** | Análisis del PO | **¿Con qué criterio y periodicidad se consultan las operaciones concertadas?** ¿Por cuenta al cierre de jornada, por símbolo bajo demanda, ambas? ¿La reconciliación de `FIX-2.07` alcanza también a las operaciones o sólo a las órdenes? | Sin esto, `FIX-2.05` no tiene definido su disparador | Definir con los referentes de negocio de BBVA en la Épica 1. Propuesta base: consulta por cuenta al cierre de jornada más consulta bajo demanda por símbolo | ⏳ **Pendiente (2026-09-16).** El PO mantiene la propuesta y la deja a confirmar con el equipo técnico o con el negocio durante la Épica 1. Registrado como nota en las historias afectadas |
| **S-07** | SUP-04 | **¿Quién dispara las consultas en la Fase 1?** Con el MVP puramente saliente, no hay cola de entrada | Define si hace falta implementar el consumo MQ en el MVP | Recomendación: el conector dispara sus consultas según su propia planificación configurable. El sentido de entrada se abre recién en la Épica 4, conforme a SUP-04 | ⏳ **Pendiente (2026-09-16).** El PO mantiene la propuesta y la deja a confirmar con el equipo técnico o con el negocio durante la Épica 1. Registrado como nota en las historias afectadas |
...
11. **Contradicción de gestión de contraseñas.** La propuesta técnica describe una estrategia de rotación diaria mediante «un mensaje específico de cambio de contraseña» con contraseña actual y nueva. **Ese mensaje no existe en los ROE de A3**: el `Logon` sólo define `Username` (553) y `Password` (554), y no hay tag `NewPassword` (925) ni mensaje de cambio de credenciales en la lista de mensajes soportados. ✅ Resuelto: la gestión es administrativa y fuera de banda (`S-02`).
...
|---|---|---|---|---|---:|
| `FIX-2.01` Iniciar sesión | `FIX-2.09`, `FIX-2.12` | `35=A` ↑↓, `35=5` ↓, `35=3` ↓ | Evento de sesión (MSG-01, MSG-02, MSG-19, MSG-23, MSG-24) | RN-06, RN-07, RN-09, RN-10, RN-13, RN-14, RN-18 | 1 |
...
**Sin huérfanos:** los 24 códigos del §5 están referenciados al menos por una historia, y todas las historias de §6 y §7 tienen al menos un código asociado o una justificación de por qué no lo necesitan (`FIX-2.09`, `FIX-2.12` y `FIX-2.14` operan por debajo del nivel de evento publicado).
...
| Recomendaciones del PO (§10) | **12** |
| Reglas de negocio transversales (§4) | **18** |
| Códigos de evento y error (§5) | **24** |
| Spikes y decisiones (§9) | **16** — 4 resueltos, 12 pendientes |
| **Total de ítems cargables a backlog** | **58** (8 HU + 6 HT + 12 cabeceras + 20 tareas + 12 recomendaciones) |
