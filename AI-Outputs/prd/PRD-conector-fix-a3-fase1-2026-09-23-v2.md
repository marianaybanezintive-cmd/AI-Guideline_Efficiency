...
> **Autor:** Product Owner
> **Fuentes:**
> - `ImplementacionFixel cliente_PropuestaTecnica_20260908 - DRAFT.docx` (propuesta técnica intive v2.0, 08-sep-2026)
...
El MVP de TCR/ER es, en una frase: **el conector observa el mercado y lo publica hacial cliente; todavía no actúa sobre él.**
...
- **Entrega valor propio, no es un andamio.** Con el MVP el cliente obtiene un *feed* confiable de ejecuciones y operaciones hacia sus sistemas internos aunque las órdenes se sigan originando por eTrader u otro canal. Es un entregable con valor aun si la Épica 4 se demorara.
...
### Segunda ronda de definiciones — 2026-09-16

| Decisión | Definición confirmada | Qué cambia |
|---|---|---|
| **Origen de las credenciales** | **Las contraseñas se reciben por una API del banco.** No hay lectura de un secret ni de una bóveda | El conector consulta la API al abrir la sesión de cada jornada y al reanudar tras un *hard stop*, y mantiene la credencial sólo en memoria. Una rotación de contraseña se propaga **sin redesplegar**. Contrapartida: aparece una **dependencia de disponibilidad en el arranque** que antes no existía. Si la API no responde a las 09:30, el conector no abre sesión y la jornada arranca ciega |
| **Números de secuencia** | **Reinicio diario.** Las secuencias vuelven a 1 al abrir la sesión de cada jornada | La persistencia pasa a tener alcance **intra-jornada**: sirve para sobrevivir a un reinicio del proceso dentro del día, no para dar continuidad entre días. La detección de gaps, el `Resend Request` y la ventana de deduplicación quedan acotados a la jornada |
| **Persistencia de secuencias** | **Por archivos, deliberadamente para no impactar la base de datos** | **Sybase sale del alcance del MVP.** Desaparecen la dependencia de Stored Procedures para el flujo de sesión, el riesgo de caudal asociado y una dependencia de equipo del cliente |
...
### Tercera definición — revisada el 2026-09-18: quién dispara las consultas

Entre el 16 y el 18 de septiembre se evaluó incorporar al MVP una **cola de entrada** para que el cliente disparara las consultas enviando comandos por MQ. **Esa definición quedó desestimada.**

**El MVP es puramente saliente.** Su flujo es, de punta a punta:

1. A3 envía los reportes **TCR y ER** hacia la sesión del conector.
2. El conector los **recibe, procesa y normaliza**.
3. El conector los **deposita en la MQ del cliente**.

No hay consumo desde MQ en la Fase 1. Las consultas que el conector necesite emitir hacia A3 (`35=H`, `35=AF`, `35=AD`) las dispara **por su propia lógica configurable**, no por pedido externo. Todo el sentido de entrada —tanto comandos de consulta como instrucciones de orden— **se trata en la Épica 4** y no se analiza en esta fase.

**Consecuencia:** el alcance de la Épica 2 vuelve al estado previo. No hay historia de consumo de comandos, no hay contrato MQ de entrada y el componente consumidor de MQ vuelve a la Épica 4. La re-estimación de la Épica 2 no incorpora alcance nuevo por este concepto.

### Indisponibilidad de la API de credenciales — 2026-09-16

Ante una API de credenciales caída, el conector aplica **reintento acotado y alerta crítica, sin abrir sesión y sin degradar a credenciales almacenadas localmente**, que es precisamente lo que la API viene a evitar.
...
el cliente está construyendo las capacidades para operar por cuenta propia y de terceros en el mercado de capitales argentino bajo la figura de ALyC, con evolución gradual. Esa operatoria requiere integración estandarizada con los mercados, tanto para el ciclo de vida de las órdenes como para la información de mercado.

El proyecto construye un **conector FIX** que actúa como punto de adaptación entre el ecosistema interno del cliente —que se comunica por **colas MQ**— y la interfaz **FIX 5.0 SP2** del gateway **FIX-PTP** de A3 Mercados. El conector encapsula la gestión de sesión y el procesamiento de mensajes del protocolo, y separa las capacidades comunes de FIX de las particularidades de A3 para habilitar futuras integraciones con otros mercados.

el cliente fijó la prioridad: **login, logout, heartbeat, TCR y ER primero**. El objetivo de negocio de la Fase 1 no es operar: es **habilitar al banco la subida a producción de la integración con A3**, con la homologación aprobada, antes del 31-dic-2026.
...
Habilitar al cliente a **subir a producción la integración con A3** con homologación aprobada (aunque sea de alcance parcial), antes del 31-dic-2026, sin haber implementado todavía el ruteo de órdenes ni el consumo de *market data*.
...
| OBJ-2 | Ningún evento del mercado se pierde ni se duplica hacial cliente | Ante corte y reconexión de sesión, el conjunto de ER y TCR publicados en MQ coincide exactamente con el emitido por A3, sin duplicados, verificado por conciliación |
| OBJ-3 | Contrato MQ de salida definido, versionado y estable | Los contratos de los mensajes publicados están aprobados por arquitectura del cliente y no cambian entre el fin de Épica 1 y el fin de Épica 3 |
...
| OBJ-6 | Conformidad funcional del cliente | Pruebas de aceptación ejecutadas por el cliente sobre el ambiente intive, con evidencia y conformidad firmada |
...
| Tiempo desde recepción del mensaje FIX hasta publicación en MQ | p95 ≤ 500 ms (a validar contra el estándar del cliente en Épica 1) |
...
- **No se consume nada desde MQ.** El MVP es puramente saliente: recibe de A3 y publica hacial cliente. Todo el sentido de entrada es Épica 4.
...
|---|---|---|
| **Sistemas internos del cliente** (consumidores) | Aplicaciones que necesitan conocer estados de órdenes y operaciones ejecutadas | Consumen los mensajes que el conector publica en MQ |
...
| **Operador de mesal cliente** | Usuario de eTrader | En Fase 1, genera el tráfico de órdenes que produce los ER que el conector observa |
| **Equipo de operaciones / soporte** | Monitoreo de la conectividad | Consume métricas, logs y alertas de sesión |
| **Arquitectura y seguridad el cliente** | Define estándares, MQ, secretos | Aprueba contratos, framework MQ y esquema de rotación de credenciales |
...
| **Motor de normalización** | Traduce mensajes FIX al modelo canónico del cliente | 2 |
| **Integración MQ (salida)** | Publica eventos normalizados hacia las colas del cliente | 2 |
| **Integración MQ (entrada)** | Consume comandos e instrucciones desdel cliente | **4** — fuera del MVP |
...
| **Gestión de secretos** | Lectura del secret de credenciales administrado por seguridad el cliente | 1 y 2 |
...
| **Security List Request / Security List** | `x` / `y` | ↑↓ | **5, con excepción a evaluar** | ⚠️ **Zona gris.** Es el catálogo de instrumentos. Los ER y TCR traen `Symbol` como string; si la normalización hacial cliente necesita enriquecer con datos del instrumento (moneda, tipo, vencimiento, segmento), este mensaje debe adelantarse al MVP. Es solo lectura, así que no viola la regla del corte. Ver [S-04](#12-riesgos-dependencias-y-decisiones-abiertas) |
...
|---|---|---|
| **Conector → Cliente (publicación)** | **2 — MVP** | Eventos de estado de orden derivados de ER, operaciones ejecutadas derivadas de TCR, estado de sesión, estado de mercado, y errores. **Este contrato se define completo en la Épica 1 y no cambia en la Épica 4** |
| **Cliente → Conector (cualquier sentido de entrada)** | **4** ✅ definido 18-09 | **Fuera del MVP.** Se evaluó incorporar comandos de consulta al MVP y quedó desestimado: el MVP no consume de MQ. Tanto los comandos de consulta como las instrucciones de orden se tratan en la Épica 4 |
| **Cliente → Conector (instrucciones de orden)** | **4** | Alta, cancelación y modificación de órdenes. Es el sentido que abre el riesgo: requiere idempotencia, deduplicación ante *redelivery* de MQ, generación de `ClOrdID` único y máquina de estados |
...
2. **Contratos MQ preliminares** para el sentido conector → el cliente: eventos de orden, operaciones, estado de sesión, estado de mercado y errores. Incluye la política de correlación de identificadores.
3. **Arquitectura de referencia**: separación entre capacidades comunes FIX, adaptador específico de A3 y adaptador de integración el cliente. Diseño del FIX Engine y decisión sobre la librería base.
4. **Estrategia de uso de MQ**: framework aprobado por el cliente, colas, productores y consumidores, tratamiento de errores, *dead letter*, comportamiento ante *redelivery*.
5. **Estrategia de observabilidad y de logging**, alineada a los estándares y herramientas disponibles en el cliente.
...
11. **Relevamiento de la estructura de códigos del cliente** requerida por el bloque `Parties` de A3: código CNV de agente de negociación (`PartyRole=1`), Client ID (`3`), agente de compensación (`4`), *Executing Trader*, *Desk ID* (`76`) y *Customer Account* (`24`).
...
**Criterio de salida:** contratos MQ aprobados por arquitectura del cliente, ambientes accesibles con conexión FIX establecida contra reMarkets, spikes bloqueantes S-01 a S-04 resueltos, y backlog de la Épica 2 refinado y estimado.
...
### Épica 3 — Homologación A3 y Pruebas de Aceptación del cliente

**Objetivo:** obtener la aprobación de A3 sobre el alcance implementado y la conformidad funcional del cliente, para habilitar la subida a producción.
...
3. **Pruebas de aceptación del cliente** sobre el ambiente intive, con evidencia documentada.
4. **Corrección de defectos** surgidos de ambas instancias.
5. **Soporte al cliente** para la configuración de su ambiente, el despliegue del entregable parcial y su integración con el ambiente de pruebas de A3.
...
**Criterio de salida:** certificado de A3 emitido (o acta de alcance parcial acordada y firmada), defectos de homologación y de aceptación cerrados, y conformidad funcional del cliente obtenida.
...
| Bloque | Contenido |
|---|---|
| Flujo MQ → FIX | Consumo de instrucciones desde las colas del cliente, validación de estructura y obligatoriedad, rechazo de mensajes inválidos, transformación a FIX |
...
**Dependencias que no existen en la Fase 1:** definición completa de reglas de negocio por producto con los referentes de negocio del cliente, y —si aplica DMA— un sistema homologado de control de riesgo *pre-trade*.
...
**Consideración de gateway:** el gateway **FIX-PTP** (el del MVP) entrega *market data* en modalidad **Full Refresh** con profundidad máxima 10 y frecuencia de 1 mensaje cada 500 ms por símbolo, apto para terminal humana u OMS. Si el cliente necesitara mayor frecuencia (FIX-PTP-HR, 1 msg/50 ms) o *incremental refresh* de baja latencia (FIX-PTP-LL, sólo disponible vía *colocation*), eso implica **otro gateway, otra solicitud a A3 y posiblemente otra certificación**. Conviene definirlo en la Épica 1 aunque se implemente en la 5, porque condiciona la conectividad contratada.
...
| RF-02 | Las credenciales (`Username`, `Password`) se obtienen de una **API del banco**, al abrir la sesión de cada jornada y al reanudar tras un *hard stop*. Se mantienen sólo en memoria durante la jornada y nunca se persisten ni se registran en logs |
| RF-02.1 | Si la API de credenciales no responde o devuelve error, el conector no intenta la conexión, aplica reintento acotado y alerta. No degrada a credenciales almacenadas localmente |
| RF-02.2 | Ante un `Logout` (`35=5`) cuyo `Text` (58) indique credencial inválida, expirada o usuario bloqueado, el conector aplica **hard stop**: detiene todo reintento automático para no bloquear el usuario, y emite alerta crítica. La reanudación exige acción manual de un administrador |
...
| ID | Requerimiento |
|---|---|
| RF-09 | El conector persiste los `MsgSeqNum` entrantes y salientes **en archivos**, de forma que sobrevivan a un reinicio del proceso **dentro de la jornada**. No se accede a la base de datos |
| RF-09.1 | Los números de secuencia **se reinician en cada jornada**. Si los valores persistidos corresponden a un día anterior, el conector los descarta y arranca en 1 |
...
| ID | Requerimiento |
|---|---|
| RF-20 | El conector emite `Trade Capture Report Request` (`35=AD`) con `TradeRequestType=1` y `TrdType=0` (Regular Trade), en modalidad por cuenta y por símbolo. El flujo es exclusivamente *pull*: **no existe suscripción a TCR**, y la consulta se emite de forma **asincrónica a demanda**, sin periodicidad fija |
...
| RF-29 | El conector consume `News` (`35=B`) y lo publica hacial cliente asociado a su `MarketSegmentID` |
...
### 8.8 Seguridad, operación y configuración

| ID | Requerimiento |
|---|---|
| RF-37 | El conector enmascara de forma irreversible las credenciales y datos sensibles (tag 554 y similares) en todo registro de auditoría, traza y mensaje de error, antes de escribirlo en cualquier destino. El mensaje FIX que viaja por el protocolo no se modifica |
| RF-38 | El conector gestiona automáticamente el ciclo diario de la sesión: apertura a las 09:30, cierre controlado a las 19:00, reinicio de secuencias, reconciliación de inicio de jornada y generación de resumen diario. Los horarios son configurables por ambiente |
| RF-39 | El conector aplica control de caudal propio antes de emitir solicitudes hacia A3, respetando los límites documentados (1 solicitud masiva/s, 100 individuales/s), encolando las solicitudes que excedan el límite |
| RF-40 | El conector expone un mecanismo de healthcheck que informa estado de sesión FIX, estado de conexión MQ, última actividad y secuencias actuales |
| RF-41 | Toda la configuración operativa del conector (endpoints, CompIDs, intervalos, colas, cuentas, ventana operativa) es externalizable por ambiente sin recompilación |
| RF-42 | El contrato de los eventos publicados en MQ incluye un bloque de instrumento opcional y versionado, diseñado para que el enriquecimiento futuro con datos de SecurityList sea una extensión aditiva que no rompa a los consumidores existentes |
...
| Persistencia (si se requiere) | Sybase 15.7/16 con Stored Procedures. ⚠️ **Fuera del alcance del MVP** desde el 2026-09-16: las secuencias de sesión se persisten **en archivos**, deliberadamente para no impactar la base de datos |
| Mensajería interna | Colas MQ; el framework/librería concreto lo define el cliente en la Épica 1 |
| Gestión de credenciales | **API del banco** que entrega las credenciales de A3, consultada al abrir sesión y al reanudar tras *hard stop*. La credencial se mantiene sólo en memoria. ✅ Definido el 2026-09-16 |

> ⚠️ Verificar en la Épica 1 que la versión de WebSphere Liberty disponible en el cliente soporte Java 25. Es una combinación que conviene validar con un *spike* de despliegue temprano antes de comprometer código.
...
| RNF-03 | La arquitectura separa las capacidades comunes de FIX, el adaptador específico de A3 y la integración con el cliente, de modo que incorporar otro mercado FIX no requiera modificar el núcleo |
| RNF-04 | Las credenciales nunca aparecen en logs, trazas, mensajes de error ni en el repositorio |
| RNF-05 | El conector soporta el reinicio del proceso sin pérdida de las secuencias de sesión **dentro de la jornada**; entre jornadas las secuencias se reinician |
...
| RNF-08 | El conector no intenta conectarse ni genera alertas por desconexión fuera de la ventana de sesión configurada |
| RNF-09 | Las credenciales no aparecen sin enmascarar en ningún registro, traza, mensaje de error, ni artefacto desplegado |
...
| **H-3** | **`ResetSeqNumFlag` (141) no está documentado en el ROE** | ✅ **Resuelto el 2026-09-16: el reinicio de secuencias es diario.** La persistencia pasa a tener alcance intra-jornada. Ver §1.bis y **S-05** |
...
| **H-5** | **El bloque `Parties` de los TCR por cuenta exige códigos CNV específicos del cliente**: agente de negociación (`PartyRole=1`), Client ID (`3`), agente de compensación y liquidación (`4`), Executing Trader, Desk ID (`76`) y Customer Account (`24`) | Sin esos códigos no se puede emitir un `AD` por cuenta. Es una dependencia dura del cliente para el MVP, y no figura en el backlog borrador |
...
| **O-6** | **Las épicas de ambientes (9, 10 y 11) suman entre 34 y 63 SP**, sobre un total de 118–228 SP. Es entre el 28% y el 29% del esfuerzo, y **depende del cliente, no del equipo de desarrollo** | Tratarlas como dependencias con dueño y fecha explícitos, no como backlog del equipo. Son el principal riesgo de cronograma |
...
| **S-04** ⏳ **SIN DEFINIR** | **¿La normalización de ER y TCR hacia el modelo canónico del cliente requiere enriquecimiento con datos del instrumento?** Los ER y TCR traen `Symbol` como string | Si la respuesta es sí, hay que adelantar `SecurityList` (`x`/`y`) al MVP. Es solo lectura, así que no rompe el corte, pero sí agrega alcance | ⏳ **Sin definir al 2026-09-16 («no sabemos»).** Para que la indefinición no bloquee el contrato MQ —que según OBJ-3 no debe cambiar entre el fin de la Épica 1 y el fin de la Épica 3— se diseña el evento normalizado con un **bloque de instrumento opcional y versionado**, de modo que incorporar `SecurityList` más adelante sea una extensión aditiva y no un cambio que rompa a los consumidores |
| **S-05** ✅ **RESUELTO 16-09** | **¿Cómo se manejan los números de secuencia entre jornadas?** ¿Se reinician a 1 al abrir la sesión diaria o continúan? `ResetSeqNumFlag` (141) no está documentado en el ROE | Afecta directamente el diseño de la persistencia y la lógica de detección de gaps | ✅ **RESUELTO 16-09: reinicio DIARIO.** Ya no se necesita configurabilidad. La persistencia pasa a alcance intra-jornada y descarta los valores de días anteriores |
...
| **S-07** ✅ **RESUELTO 16-09** | **¿Dónde se persisten las secuencias?** El Excel pregunta «validar con el cliente dónde se persiste (storage?)» y anota «se utiliza file storage» | Define si se necesita Sybase en el MVP o alcanza con almacenamiento en archivos | ✅ **RESUELTO 16-09: por archivos, deliberadamente para no impactar la base de datos.** Sybase queda fuera del alcance del MVP |
| **S-08** | **¿Qué framework MQ apruebal cliente?** | Bloquea toda la implementación de publicación del MVP | Definición del cliente, comprometida en la propuesta para las primeras etapas. Debe cerrarse antes del fin del sprint 1 |
| **S-09** ✅ **RESUELTO 18-09** | **¿Se incluye el sentido MQ → conector para comandos de consulta en el MVP?** | Define si el MVP tiene una cola de entrada o es puramente saliente | ✅ **No: el MVP es puramente saliente.** Se evaluó incorporar una cola de comandos de consulta y quedó **desestimado**. El flujo es A3 → conector → MQ del cliente, en un solo sentido. Todo el sentido de entrada se trata en la Épica 4 |
...
| **R-6** | **Java 25 sobre WebSphere Liberty** puede no estar soportado en la versión disponible en el cliente | Medio | Spike de despliegue «hola mundo» en la Épica 1, antes de comprometer código |
| **R-7** | **Disponibilidad de referentes de negocio y técnicos del cliente** para resolver definiciones | Medio | Designar interlocutores desde el inicio y mantener una instancia regular de resolución de dudas |
...
| Ambiente el cliente: Liberty + MQ | Épica 3 (despliegue del entregable parcial) | Fin Épica 2 |
...
| **Épica 3** — Homologación A3 y Aceptación del cliente | ≥ 1 sprint | mediados de nov → ¿? | Certificado A3 (parcial) y conformidad el cliente |
...
2. **Definir con el negocio qué cuentas y operadores del cliente quedan asociados a la sesión.** Determina qué eventos verá el conector, y es precondición de la gestión anterior.
3. **Asignar dueño y fecha a los ambientes del cliente** (R-2). Representan cerca del 29% del esfuerzo estimado y no los ejecuta el equipo de desarrollo.
...
- [ ] Las credenciales se obtienen del secret del cliente y no aparecen en ningún log ni traza.
...
- [ ] el cliente ejecutó sus pruebas de aceptación sobre el ambiente intive y dejó constancia de conformidad funcional.
...
- [ ] Las credenciales y datos sensibles están enmascarados en todos los registros de auditoría.
- [ ] El ciclo diario de la sesión (apertura/cierre) se ejecuta de forma predecible dentro de la ventana configurada.
- [ ] El healthcheck del conector está operativo y responde correctamente dentro y fuera de la ventana de sesión.
